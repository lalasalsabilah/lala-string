...
async def generate_session(bot, msg, telethon=False):
    ...
    try:
        ...
        return
    try:
        phone_code_msg = await bot.ask(user_id, "periksa kode OTP dari akun telegram resmi. kirim kode kesini setelah membaca format dibawah ini, jika kode OTP. \nIf OTP is `12345`, **kirim seperti** `1 2 3 4 5`.", filters=filters.text, timeout=600)
        if await cancelled(api_id_msg):
            return
    except TimeoutError:
